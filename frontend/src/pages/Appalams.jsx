...
import {
  ShoppingCart, Heart, Search, SlidersHorizontal,
  Star, ShieldCheck, Leaf, Flame, Package, Truck, X,
  ArrowRight, Award
} from 'lucide-react';
...
function SEOMeta() {
  React.useEffect(() => {
    document.title = 'Appalams & Papads | Sharadha Stores — Homemade Traditional Crispy Appalams';
    const desc = document.querySelector('meta[name="description"]');
    if (desc) {
      desc.setAttribute('content', 'Buy authentic homemade Appalams, Rice Appalam, Pepper Appalam, Kerala Papadam and traditional Papads from Sharadha Stores. No preservatives. Freshly prepared. Pan India delivery.');
    }
  }, []);
  return null;
}
...
const FEATURES = [
  { icon: <ShieldCheck size={26} />, title: 'Homemade Quality', desc: 'Every appalam hand-pressed and sun-dried in our home kitchen.' },
...
export default function Appalams() {
  ...
  /* ── Get only Appalam products ── */
  const allAppalams = useMemo(() => {
    return getLocalProducts().filter(p => p.category === 'Appalams');
  }, []);

  /* ── Apply filters ── */
  const filtered = useMemo(() => {
    let list = [...allAppalams];
    ...
    switch (sortBy) {
      ...
      case 'newest':    list.sort((a, b) => b._id.localeCompare(a._id)); break;
      default: /* popular — bestsellers first */
        list.sort((a, b) => (b.isBestSeller ? 1 : 0) - (a.isBestSeller ? 1 : 0));
    }
    return list;
  }, [allAppalams, search, minPrice, maxPrice, sortBy, onlyInStock]);
      ...
      <div style={{ backgroundColor: 'var(--background)', color: 'var(--text)', fontFamily: 'var(--font-body)' }}>
        ...
        <section style={{
          ...
          <div style={{ position:'relative', zIndex:2, maxWidth:'780px' }} className="slide-up">
            ...
            <p style={{
              ...
              Crispy, authentic, handmade appalams prepared using traditional family recipes — sun-dried, no preservatives, delivered fresh to your doorstep.
            </p>
            ...
            <div style={{ display:'flex', gap:'1rem', justifyContent:'center', flexWrap:'wrap' }}>
              ...
              onMouseLeave={e=>{e.currentTarget.style.transform='none';e.currentTarget.style.boxShadow='0 8px 24px rgba(245,158,11,0.45)';}}>
                Shop Appalams <ArrowRight size={17} />
              </a>
              <Link to="/shop" style={{
            ...
            <div style={{
              ...
              {[
                ['🧑‍🍳', `${allAppalams.length} Varieties`, 'Available'],
                ['⭐', '4.8+', 'Avg Rating'],
                ['🚚', 'Pan India', 'Delivery'],
                ['🌿', '0%', 'Preservatives'],
              ].map(([icon, val, label]) => (
        ...
        <section id="products" style={{ padding:'4rem 0 5rem' }}>
          <div className="container">
            ...
            <div style={{ textAlign:'center', marginBottom:'2.5rem' }}>
              ...
              <h2 style={{ fontFamily:'var(--font-heading)', fontSize:'2.4rem', fontWeight:500, color:'var(--secondary)', marginTop:'0.5rem' }}>
                Our Appalam Collection
              </h2>
              <div style={{ width:'50px', height:'2px', background:'var(--primary)', margin:'0.75rem auto 0' }}/>
            </div>
            ...
            <div style={{
              ...
              <div style={{ position:'relative', flex:'1 1 220px' }}>
                ...
                <input
                  type="text" placeholder="Search appalams..."
            ...
            {filtered.length === 0 ? (
              <div style={{
                ...
                <p>No appalams match your filters.</p>
                <button onClick={clearFilters} style={{
                  ...
                  return (
                    ...
                    >
                      ...
                      <div style={{ position:'absolute', top:'10px', left:'10px', display:'flex', flexDirection:'column', gap:'4px', zIndex:3 }}>
                        ...
                        {lowStock && (
                          <span style={{ background:'#f59e0b', color:'#fff', fontSize:'0.65rem', fontWeight:700, padding:'2px 7px', borderRadius:'3px' }}>LOW STOCK</span>
                        )}
                      </div>
                      ...
                      <Link to={`/product/${product._id}`} className="product-img-wrap" style={{ height:'210px', display:'block', textDecoration:'none', flexShrink:0 }}>
                        <img
                          ...
                          onError={e => { e.currentTarget.src='/images/appalam-special.png'; }}
                        />
                      </Link>
                      ...
                      <div style={{ padding:'1.1rem', display:'flex', flexDirection:'column', flexGrow:1 }}>
                        ...
                        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'0.4rem' }}>
                          <span style={{ fontSize:'0.7rem', textTransform:'uppercase', color:'var(--secondary)', fontWeight:700, letterSpacing:'0.5px' }}>Appalam</span>
                          {product.weight && (
                          ...
                          <div>
                            {hasDiscount ? (
                              <div style={{ display:'flex', alignItems:'baseline', gap:'0.4rem' }}>
                                ...
                                <span style={{ fontSize:'0.8rem', textDecoration:'line-through', color:'var(--text-muted)' }}>₹{product.price}</span>
                              </div>
                            ) : (
        ...
        <section style={{
          ...
          <div className="container" style={{ textAlign:'center' }}>
            <span style={{ fontSize:'0.8rem', letterSpacing:'3px', textTransform:'uppercase', color:'var(--primary)', fontWeight:700 }}>OUR PROMISE</span>
            <h2 style={{ fontFamily:'var(--font-heading)', fontSize:'2.4rem', fontWeight:500, marginTop:'0.5rem', marginBottom:'2.5rem' }}>
              Why Sharadha Appalams Stand Apart
            </h2>
            <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(200px,1fr))', gap:'2rem' }}>
              {[
                { icon:'🌾', title:'Premium Urad Dal', desc:'Only Grade-A black gram dal sourced directly from organic farms in Tamil Nadu.' },
                { icon:'☀️', title:'Sun-Dried 48 hrs', desc:'Hand-pressed discs dried naturally under full sun — never machine-dried.' },
                { icon:'🧂', title:'Rock Salt Only', desc:'We use pure rock salt, never refined table salt, preserving natural minerals.' },
                { icon:'📦', title:'Vacuum Sealed', desc:'Packed in airtight, food-grade pouches to maintain crispness for months.' },
              ].map(p => (
          ...
          <div className="container" style={{
            ...
            <div>
              <div style={{ fontSize:'0.78rem', color:'var(--text-muted)', marginBottom:'0.4rem' }}>
                ...
                {' › '}
                <span style={{ color:'var(--secondary)', fontWeight:600 }}>Appalams</span>
              </div>
              <h3 style={{ fontFamily:'var(--font-heading)', fontSize:'1.5rem', color:'var(--secondary)' }}>
                Explore More Categories
              </h3>
            </div>
            <div style={{ display:'flex', gap:'0.75rem', flexWrap:'wrap' }}>
              {[['Sweets','🍮'],['Snacks','🥨'],['Pickles','🫙'],['Spice Powders','🌶️']].map(([name, emoji]) => (
                <Link key={name} to={`/shop?category=${name}`} style={{
